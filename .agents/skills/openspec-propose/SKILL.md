...
4. **Prepare feature branch and create the change directory**

   **Branching requirement**: Before creating the change or authoring artifacts, ensure work is isolated on a dedicated feature branch rather than the base branch (`main` or `master`):
   - Check current git branch: `git branch --show-current`
   - If on `main` or `master`:
     - Check if branch `feat/<name>` exists:
       - If not, create and checkout: `git checkout -b feat/<name>` (or `fix/<name>` for bug fixes)
       - If it already exists, switch to it: `git checkout feat/<name>`
     - Announce: "Switched to branch: `feat/<name>`"
   - If already on a dedicated feature branch matching the change, remain on it.
...
- Ensure work is on a dedicated feature branch (`feat/<name>` or `fix/<name>`), never directly on `main` or `master`
