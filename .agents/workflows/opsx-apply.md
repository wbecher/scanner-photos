...
6. **Ensure feature branch and implement tasks (loop until done or blocked)**

   **Branching requirement**: Before making code changes or marking tasks, ensure work is done on a dedicated feature branch rather than the base branch (`main` or `master`):
   - Check current git branch: `git branch --show-current`
   - If on `main` or `master`:
     - Check if branch `feat/<change-name>` exists:
       - If not, create and checkout: `git checkout -b feat/<change-name>` (or `fix/<change-name>` for bug fixes)
       - If it already exists, switch to it: `git checkout feat/<change-name>`
     - Announce: "Switched to branch: `feat/<change-name>`"
   - If already on a feature branch for this change, remain on it. Never implement tasks directly on `main` or `master`.
...
- Always ensure implementation happens on a dedicated feature branch (`feat/<change-name>`), never directly on `main` or `master`
